...
pub fn main() !void {
    ...
    var options = Options.processArgs(allocator) catch {
        std.debug.print("Usage: atlas_gen [path] --size [size=14] --output [output=./atlas.png] --json-output [json-output=./json-atlas.png]", .{});
        return;
    };
    ...
    if (c.FT_New_Face(ft_library, @ptrCast(options.input_path), 0, &face) != 0) std.debug.panic("Unable to laod {s}.", .{options.input_path});
